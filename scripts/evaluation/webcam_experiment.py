...
"""
基于真实项目代码的实验测试脚本
所有截图和数据保存到 experiments/results/ 目录
"""
...
from config.constants import BACKGROUND_DIR, EXPERIMENT_RESULT_DIR
...
# ─── 全局结果保存目录配置 ───
RESULTS_DIR = os.path.join(EXPERIMENT_RESULT_DIR, "webcam")
...
def run_experiment(algorithm_id: int,
    ...
    # 每个算法在experiments/results/webcam/下有独立子目录
    # 例如：experiments/results/webcam/MediaPipe/
    ...
    factory.load_backgrounds(BACKGROUND_DIR)
    # 调用实例方法，加载assets/backgrounds/目录下的背景图片
    # 注意：背景目录必须存在且不为空，否则process_frame直接返回原帧
...
def plot_all_results(all_results: list) -> None:
    """
    绘制所有算法的对比图表，保存到 experiments/results/webcam/ 目录
